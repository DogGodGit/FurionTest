...
public class ArrayDequeTests
{
    ...
    /// <summary>
    /// 测试队列溢出时的行为，验证是否正确限制队列大小。
    /// </summary>
    [Fact]
    public void Test_Overflow()
    {
        var deque = new ArrayDeque(2);
        deque.PushLast(10);
        deque.PushLast(20);
        deque.PushLast(30); // 应该不会添加
        Assert.Equal(2, deque.Size());
        Assert.Equal(new[] { 10, 20 }, deque.ToArray());
    }
